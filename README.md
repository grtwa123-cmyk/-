...
| --- | --- | --- |
| **Measured** | The mechanism runs and the textbook result is read back out of it. Nothing on the page types the answer in. | 26 |
| **Integrated** | The equations of motion are stepped forward in time. What you see is where they go. | 3 |
| **Real data** | A three-dimensional structure built from measured constants — geometry rather than simulation. | 4 |
| **Solved** | The governing equation is solved numerically every frame, rather than pieced together from special cases. | 1 |
...
against its closed form on every commit — 27 of the 36 so far. It is not stored
...
| **Doppler Effect** | The shift is read off the picture rather than evaluated: where consecutive rings cross the line of travel is where the wavefronts are, and the gap between those crossings is the wavelength — **(c − v)/f** ahead and **(c + v)/f** behind, to a part in 10¹³ across ten settings, with every pair of rings agreeing with every other to 10⁻¹³. Past Mach 1 the rings become tangent to a cone and the measured half-angle is **arcsin(1/M)** to the same precision; the spread *across* rings is reported too, because "every ring gives the same angle" is what makes it a cone rather than a coincidence. The forward readouts are then withdrawn, because a source that has outrun its own waves has nothing ahead of it to have a wavelength — the closed form says so by going negative. This page was built and reverted three times. Two things finally made it work, and both are asserted: retiring every ring in flight when a control moves (they belong to the old setting) and when the source bounces (they belong to the other leg); and recording each ring at the instant it left rather than at the end of the integration step, which misplaced every centre by up to v·dt — invisible below Mach 1, and at Mach 2 the difference between rings tangent to one cone and a half-angle 1.7% out with 4% scatter. |
| **Newton's Cannon** | One law is written down, a = −GM·r̂/r², and the two speeds the thought experiment is about are *extrapolated from five ordinary shots* rather than evaluated. Fire five cannonballs that all stay in orbit, measure how high each gets, and plot 1/r_apogee against 1/v₀²: the points fall on a straight line, residual 3×10⁻⁸. Where the line reaches zero the apogee is unreachable — escape speed, 8.2339. Where it reaches 1/r₀ the apogee is back at the muzzle — a circle, 5.8222. √(2GM/r₀) and √(GM/r₀) agree to 0.02 ppm, and √ is written nowhere in the derivation. Fired at the measured circular speed the radius then holds to four parts in 10⁸ for a whole orbit. The apsides are found by watching r turn around and fitting a parabola through the bracketing samples — worth doing for the *time*, not the radius, since r is flat at a turn but the period is twice that time. Velocity Verlet at a fixed step keeps the energy bounded at a few parts in 10⁸ and the angular momentum exact, so equal areas take equal times to fifteen digits. |
| **Gravity & Orbits** | Kepler's three laws, fitted to planets you launch by hand. Each planet's semi-major axis comes from its own two turning points and its period from the gap between them; every orbit that closes drops a point on a log–log chart, and a least-squares line through them comes out at slope **1.4999999** with the points 3×10⁻⁷ from it in log. The intercept is worth as much as the slope: it is 2π/√GM, and it gives back the star's GM to about a part per million at every mass from 0.2× to 20× — twenty times the mass really does measure twenty times the GM. Kepler's first law is held in its sharp form, that the ellipse does not *turn*: the measured perihelion drift stays under 10⁻⁵ rad per orbit, and changing the exponent by a thousandth is what moves it first. The reachable orbits span a factor of 300 in period and no single step serves them — at a fixed 10 ms the tight fast corner gets 171 steps per orbit and precesses a hundredth of a radian per turn under an inverse-square force — so each planet carries its own fixed step, sized at launch from the periapsis its conserved E and L imply. |
| **Impulse & Force (Egg Drop)** | The old version of this page drew its answer: it set the collision time per material, took the impulse to be mv, and drew a half-sine of height πmv/2t. Now the cushion is a real contact — F = k·u·(1 + a·u̇), Hunt–Crossley so the force starts at zero instead of jumping and can never pull the egg back down — integrated with RK4. The force curve is produced, and the impulse is the trapezoidal area swept out under it as it happens. That area equals the momentum removed to **under a part per million** across three cushions and the whole slider, with the weight the cushion worked against for the duration counted on the other side. Switch the damping off and the integrator reproduces the two exact answers a spring under gravity has — contact time (2/ω₀)(π − arctan(v₀ω₀/g)) and peak mg + √((mg)² + kmv₀²) — to the same precision. The lesson is then measured rather than asserted: the same 50 cm drop onto the three floors removes 0.305, 0.314 and 0.345 N·s, but peaks at 121 N, 22.9 N and 7.2 N. And the old page's peak ≈ (π/2)·average turns out to hold only in the gentle limit: the measured ratio is 1.60 from 2 cm and 4.53 from 12 m. |
...
| **Acid–Base Titration** | The pH was already honest — one exact charge balance bisected at every point, no regime special-cased. What the panel *printed* was Cₐ·Vₐ/C_b labelled "the equivalence volume", which is the answer rather than a measurement of it. It is now **located** the way a chemist locates it, as the steepest point of the curve by golden section on |dpH/dV|, and lands on Cₐ·Vₐ/C_b to a fraction of a part per million across both acids and every setting — with a fine scan confirming nowhere on the curve is steeper. Two more results come off the same curve: the pH halfway there is **pKa** to 7×10⁻⁴ while the concentrations move fourfold, and the pH at it is 7.000 for the strong acid and 7 + ½pKa + ½log C for the weak one. Henderson–Hasselbalch holds across the plateau and is shown failing where it should — the error is 47× larger a tenth of the way in than nine tenths, because it neglects the acid's own dissociation. The one readout that could not have come from a formula is the steepness itself: 4 300 pH/mL for the strong acid against 82 for the weak, which is what decides whether an indicator can find the endpoint at all. |
...
| **Predator & Prey** | The Lotka–Volterra equations integrated with RK4, and then *measured* — the page used to print γ/δ as "the equilibrium" and 2π/√(αγ) as "the period", and neither survived being checked. The equilibrium is not where the populations sit, because they never sit anywhere; it is what they **average to**. Volterra's theorem says the time average of the prey over one whole cycle is exactly γ/δ and of the predators α/β, and accumulating ∫x dt between two prey peaks reproduces that to a part in 10⁴ on cycles from a whisper around the fixed point out to five times its size, across parameter sets spanning 1 to 40 in x*. The period is not the linearised one either: measured against 2π/√(αγ) the gap is 0.01% for a tiny swing and 28% for a large one, and it closes **quadratically** — halve the amplitude and the gap quarters, which is checked as a scaling law rather than at a point. "A quarter-cycle behind" goes the same way: 0.2498 of a cycle at the smallest swing, 0.10 at the largest. V = δx − γ ln x + βy − α ln y is conserved, so its drift is a readout of integration error and nothing else — a part in 10⁸ at the step the page runs. |
...
| --- | --- |
| `lint` | Every tracked `.js` file parses, and so does every inline `<script>` — 55 KB of the site's JavaScript lives in eight of them, including the two largest scripts it ships. Blocks are picked by type, so the black hole's GLSL shader and import map are left alone. Plus `.editorconfig` conformance. |
...
| `cannon`, `circuit`, `decay`, `diffraction`, `doppler`, `electrolysis`, `gas`, `impact`, `kinetics`, `lotka`, `enzyme`, `equilibrium`, `generator`, `lens`, `neuron`, `orbit`, `pendulum`, `phases`, `photoelectric`, `projectile`, `refraction`, `resonance`, `selection`, `spectra`, `string`, `titration`, `wave` | The physics. Each simulation is checked against its closed form — Ohm's law and Kirchhoff residuals, the N-slit intensity and its missing orders, Michaelis–Menten from counted turnovers, equilibrium constants from counted reaction events against k₊/k₋ and van 't Hoff, the thin-lens equation from traced rays, Hodgkin–Huxley threshold and refractory period, Foucault precession at Ω·sin φ, a projectile's measured range against v₀²sin2θ/g and the optimal launch angle found by sweep, Planck's constant fitted from counted stopping voltages, the Rydberg constant fitted from emitted lines, Faraday's 2 : 1 counted out of the bubbles, Snell's law read off a Huygens wavelet envelope and cross-checked against Fermat's least-time path, an AC waveform and its quarter-turn phase produced by differentiating an integrated flux, Lennard-Jones melting, condensation and Maxwell–Boltzmann relaxation, the resonant amplitude and phase measured back out of the motion, Wright–Fisher against the infinite-population recursion, counted survivors held to N₀·2^(−t/T½) against the binomial spread of the count itself, the gas laws read off an impulse tally rather than evaluated, the Boltzmann factor counted out of hard-disc collisions and compared at the temperature the gas actually reaches rather than the one on the dial, harmonics found by transforming a plucked string rather than written down, located interference fringes held to the hyperbola they sit on rather than to the small-angle form of it, a diffraction pattern summed over the aperture and then searched, with the grating equation read off the peaks the search returned, Kepler's 3/2 and the star's GM both fitted to hand-launched planets rather than printed beside them, the impulse of a collision taken as the area swept under a produced force curve and held against the momentum it removed, a predator-prey cycle whose measured time averages land on the fixed point neither population ever sits at, and the circular and escape speeds extrapolated from five measured apogees rather than evaluated — the fit is recomputed inside the suite from the page's own measured points, so a hard-coded constant does not survive it. Where a check is statistical the bound is derived from the run's own counts, not picked. Where an approximation is on screen it is held to its own error term, not waved through: `wave` fails if λL/d is out by anything other than √(1 − (λ/d)²) ⁄ √(1 + (d² − λ²)/4L²), and `diffraction` fails if the aperture sum has not converged — each doubling of the sample count has to quarter the change, because a sum that has not converged is a formula with extra steps. |
...
├── i18n/
│   ├── en.js ko.js zh.js       Full dictionaries — the source, and the fallback
│   └── pages/<lang>/<page>.js  Per-page chunks, generated
├── tools/
│   ├── build-font.py           Regenerates the Pretendard subsets (manual, rare)
│   └── split-i18n.py           Recuts the per-page dictionary chunks
├── assets/
...
- **i18n** is one loader file. `data-i18n="key"` on any element + `i18n.applyLang('ko')` walks the DOM, replaces text content, updates `<html lang>`, and emits a `langchange` event the landing page listens for to re-render its canvas cards. A page displays 5–115 of the 1333 keys, so it fetches only its own chunk — a few KB rather than 98. The chunks are cut from source by `tools/split-i18n.py`, and a key that misses one pulls the whole dictionary in behind it and repaints, so getting a chunk wrong costs a request rather than showing a raw key.
...
- **i18n every user-visible string.** Add the key to all three of `i18n/en.js`, `i18n/ko.js`, `i18n/zh.js`, reference it with `data-i18n` on the element, and rerun `python3 tools/split-i18n.py` so it reaches the page's chunk. A key your code assembles at runtime rather than writing out needs a line in that script's `DYNAMIC_PREFIXES`.
