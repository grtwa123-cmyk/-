...
// start lying, so the drift is a test failure rather than a silent omission.
{
  ...
  // The same failure the sitemap check exists for, one file over: index.html
  // opens three social descriptions with a count of the experiments. It said
  // 36 while the catalogue held 37 — a search engine and every share card
  // repeating a number nobody had updated. Generated by the same script now.
  {
    const html = fs.readFileSync(path.join(root, "index.html"), "utf8");
    const said = [...html.matchAll(
      /<meta (?:name|property)="(?:[a-z:]*)description" content="(\d+) hands-on/g)]
      .map((m) => Number(m[1]));
    check("index.html's social descriptions count the catalogue it actually has",
      said.length === 3 && said.every((n) => n === EXPERIMENTS.length),
      `tags say ${said.join(", ") || "nothing"} — catalogue has ${EXPERIMENTS.length}`
      + " — run node tools/build-seo.mjs");
  }
